...
function sortCarInventory(inventory) {
  let sortedInventory = inventory.slice();
  for(let i=0; i<sortedInventory.length; i++){
    for(let j=i+1; j<sortedInventory.length; j++){
      if(sortedInventory[j].car_model<sortedInventory[i].car_model){
        let temp = sortedInventory[i];
...
function sortCarInventory2(inventory) {
  return inventory.slice().sort((car1, car2)=>{
...
function getOlderCars(inventory, maxYear) {
  let inventoryMaxYear = [];
  for(let i=0; i<inventory.length; i++){
    if(inventory[i][`car_year`] <= maxYear){
      inventoryMaxYear.push(inventory[i]);
    }
  }
  return inventoryMaxYear;
}
...
function getGermanCars2(inventory){
  const germanCars = [];
  for(let i=0; i<inventory.length; i++){
    if(isGermanCar(inventory[i])){
      germanCars.push(inventory[i]);
    }
  }
  return germanCars;
